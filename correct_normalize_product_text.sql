-- Corrected function to normalize product text - keep hyphens in part numbers as per original plan
...
BEGIN
  ...
  -- Clean up multiple hyphens but KEEP single hyphens (important for part numbers)
  normalized_text := regexp_replace(normalized_text, '-{2,}', '-', 'g');  -- Multiple hyphens -> single hyphen
  normalized_text := regexp_replace(normalized_text, '\s*-\s*', '-', 'g'); -- Normalize spacing around hyphens
  
  -- Clean whitespace again
  normalized_text := regexp_replace(normalized_text, '\s+', ' ', 'g');
  normalized_text := trim(normalized_text);

  -- Now normalize hardware terms with proper word boundaries (order matters!)
  -- Do ZP first to avoid "zinc platedated"
  ...
  -- Handle material abbreviations - avoid conflicts
  ...
  -- Handle multi-word patterns after individual words
  ...
  normalized_text := regexp_replace(normalized_text, 'zinc pl\M', 'zinc plated', 'g');

  -- Remove remaining special characters but keep important ones for part numbers
  -- Keep: letters, numbers, spaces, periods, forward slashes, AND HYPHENS
