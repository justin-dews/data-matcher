-- Deploy working hybrid_product_match_tiered function
-- Extensions are confirmed enabled, so we can use them directly

-- Test that extension functions work
SELECT similarity('test', 'testing') as trigram_test;
SELECT levenshtein('test', 'testing') as fuzzy_test;
SELECT unaccent('café') as unaccent_test;

-- Create the hybrid matching function
...
SELECT * FROM hybrid_product_match_tiered('test product', 3, 0.1) LIMIT 3;