-- Function to normalize product text for consistent matching
...
BEGIN
  ...
  normalized_text := regexp_replace(normalized_text, '\bw/', 'with', 'g');
  normalized_text := regexp_replace(normalized_text, '\b&\b', 'and', 'g');
  normalized_text := regexp_replace(normalized_text, '\bw\b', 'with', 'g');

  -- Normalize measurement units (keep short forms for now as they're more standardized)
  -- mm, cm, inch patterns are commonly understood in industrial catalogs
  ...
  -- Normalize common hardware terms
