-- FIXED Function to get alias boost score for competitor product matching
...
BEGIN
  ...
  -- Early return if no aliases exist for this organization
  IF NOT EXISTS (
  ...
  SELECT ca.confidence_score INTO exact_match_score
  ...
  -- If exact match found, return the confidence score
  IF exact_match_score IS NOT NULL THEN
    RETURN GREATEST(0.0, LEAST(1.0, exact_match_score));
  END IF;
  ...
  -- Final explicit check: if no matches found, ensure we return 0.0
  IF NOT match_found AND exact_match_score IS NULL THEN
    RETURN 0.0;
  END IF;
